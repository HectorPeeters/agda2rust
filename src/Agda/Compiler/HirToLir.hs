...
extractGenericsFromType (LirNamedType name gs) = unique gs ++ [LirGeneric name | T.length name == 1]
extractGenericsFromType x@(LirGeneric _) = [x]
extractGenericsFromType (LirFn arg ret) = unique (extractGenericsFromType arg ++ extractGenericsFromType ret)
extractGenericsFromType LirNone = []

extractGenericsFromStmt :: LirStmt -> [LirType]
extractGenericsFromStmt (LirFunction _ generics ret _) = generics ++ extractGenericsFromType ret
extractGenericsFromStmt (LirEnum _ gs variants) = unique $ concat (map extractGenericsFromType gs ++ concatMap (\(_, xs) -> map extractGenericsFromType xs) variants)
...
instance ToLir (LirIdent, [LirType], LirExpr) [LirStmt] where
  ...
  toLir (name, argTypes, body) = do
    ...
    let types = firstType : zipWith (\t n -> let generics = unique (extractGenericsFromType firstType ++ extractGenericsFromType (argTypes !! (n - 1))) in LirFn t (LirNamedType (makeAliasName name (n - 2)) generics)) (reverse $ removeLast $ removeLast argTypes) [2 ..]
    let typeAliases = zipWith (\t n -> LirTypeAlias (makeAliasName name n) t (extractGenericsFromType t)) types [0 ..]

    let returnGenerics = unique $ concatMap extractGenericsFromStmt typeAliases
    let returnType = LirNamedType (makeAliasName name (length typeAliases - 1)) returnGenerics

    typeAliases ++ [LirFunction name (extractGenericsFromType returnType) returnType body]
...
instance ToLir HirStmt [LirStmt] where
  ...
  toLir (HirConstructor name variants) = do
    let lirVariants = map (\(n, args) -> (n, map toLir (removeLast args))) variants
    let generics =
          unique $
            concatMap (\(_, x) -> concatMap extractGenericsFromType x) lirVariants
    let enumConstructor = LirEnum name generics lirVariants

    let constructorFunctions = concatMap (\(n, ts) -> toLir (n, ts ++ [LirNamedType name []], LirDataConstructor name n [])) lirVariants
